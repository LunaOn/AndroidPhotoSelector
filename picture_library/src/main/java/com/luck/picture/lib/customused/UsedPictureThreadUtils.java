 ...
 * @describe：ThreadPool
 */
public final class UsedPictureThreadUtils {
    ...
    private static ExecutorService getPoolByTypeAndPriority(final int priority) {
        synchronized (TYPE_PRIORITY_POOLS) {
            ...
            Map<Integer, ExecutorService> priorityPools = TYPE_PRIORITY_POOLS.get((int) UsedPictureThreadUtils.TYPE_IO);
            if (priorityPools == null) {
                priorityPools = new ConcurrentHashMap<>();
                pool = ThreadPoolExecutor4Util.createPool(priority);
                priorityPools.put(priority, pool);
                TYPE_PRIORITY_POOLS.put((int) UsedPictureThreadUtils.TYPE_IO, priorityPools);
            } else {
